...
describe('Keycloak Verify Provider', () => {
  ...
  const signupProvider: KeyCloakSignUpFn = async (prof: Keycloak.Profile) => null;
  const preVerifyProvider = async (accToken: string, refToken: string, prof: Keycloak.Profile, usr: IAuthUser | null) =>
    usr;
  const postVerifyProvider = async (prof: Keycloak.Profile, usr: IAuthUser | null) => usr;
