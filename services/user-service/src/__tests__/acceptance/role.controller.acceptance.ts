...
describe('Role Controller', function () {
  ...
  it('gives status 422 when request body is invalid', async () => {
    const role = {};
    await client.post(basePath).set('authorization', `Bearer ${token}`).send(role).expect(422);
  });
  ...
  it('gives status 200 when a new role is created', async () => {
    ...
    await client.post(basePath).set('authorization', `Bearer ${token}`).send(role).expect(200);
  });
  ...
  it('gives status 204 when a role is deleted ', async () => {
    ...
    await client.del(`${basePath}/${role.id}`).set('authorization', `Bearer ${token}`).expect(204);
  });
  ...
  it('return a role objet when id is sent', async () => {
    ...
    const response = await client.get(`${basePath}/${role.id}`).set('authorization', `Bearer ${token}`).expect(200);
