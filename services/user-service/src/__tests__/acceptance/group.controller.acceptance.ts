...
describe('Group Controller', function () {
  ...
  it('gives status 422 when request body is invalid', async () => {
    ...
    await client.post(basePath).set('authorization', `Bearer ${token}`).send(group).expect(422);
  });
  ...
  it('gives status 200 when a new group is created', async () => {
    ...
    await client.post(basePath).set('authorization', `Bearer ${token}`).send(group).expect(200);
  });
  ...
  it('gives status 204 when a group is deleted ', async () => {
    ...
    await client.del(`${basePath}/${group.id}`).set('authorization', `Bearer ${token}`).expect(204);
  });
