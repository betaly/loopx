...
export class UserOperationsService {
  ...
  async create(userData: UserDto, currentUser: IAuthUserWithPermissions, options?: Options): Promise<UserDto> {
    ...
    const roleType = (RoleTypeMap[role.roleType] as RoleTypeMapValue).permissionKey;
    ...
    if (userExists) {
      ...
      if (userTenantExists) {
        ...
        if (
          ...
          currentUser.permissions.indexOf(PermissionKey.CreateTenantUserRestricted) >= 0 &&
  ...
  validateUserCreation(user: User, userData: UserDto, currentUser: IAuthUserWithPermissions, options?: Options) {
    if (
  ...
  async updateById(
    ...
    if (userData.username) {
      ...
      const userNameExists = await this.userRepository.count(whereBuilder.build());
  ...
  async updateUserTenant(userData: Partial<UserDto>, id: string, currentUser: IAuthUserWithPermissions) {
  ...
  async deleteById(currentUser: IAuthUserWithPermissions, id: string, tenantId: string): Promise<void> {
  ...
  async checkViewTenantRestrictedPermissions(
    ...
    role.forEach(r => {
      if (
        r.id &&
        currentUser.permissions.indexOf(`ViewTenant${(RoleTypeMap[r.roleType] as RoleTypeMapValue).permissionKey}`) >= 0
      ) {
  ...
  async checkForDeleteTenantUserRestrictedPermission(currentUser: IAuthUserWithPermissions, id: string) {
    if (currentUser.permissions.indexOf(PermissionKey.DeleteTenantUserRestricted) >= 0) {
      ...
      if (
        ...
        currentUser.permissions.indexOf(
          `DeleteTenant${(RoleTypeMap[userTenant.role.roleType] as RoleTypeMapValue).permissionKey}`,
        ) < 0
      ) {
        throw new AuthorizationErrors.NotAllowedAccess();
      }
  ...
  async checkForDeleteTenantUserPermission(currentUser: IAuthUserWithPermissions, id: string) {
  ...
  async checkForDeleteAnyUserPermission(currentUser: IAuthUserWithPermissions, tenantId: string) {
    if (currentUser.permissions.indexOf(PermissionKey.DeleteAnyUser) < 0 && tenantId !== currentUser.tenantId) {
      throw new AuthorizationErrors.NotAllowedAccess();
    }
  }

  async checkForUpdatePermissions(currentUser: IAuthUserWithPermissions, id: string, tenantId?: string) {
    ...
    if (
      ...
      currentUser.permissions.indexOf(PermissionKey.UpdateTenantUserRestricted) >= 0 &&
      currentUser.id !== id
    ) {
      const userTenant = (await this.utRepo.findOne({
        where: {
      ...
      if (
        !userTenant ||
        currentUser.permissions.indexOf(
          `UpdateTenant${(RoleTypeMap[userTenant.role.roleType] as RoleTypeMapValue).permissionKey}`,
        ) < 0
      ) {
        throw new AuthorizationErrors.NotAllowedAccess();
      }
    ...
    if (currentUser.permissions.indexOf(PermissionKey.UpdateAnyUser) < 0 && tenantId !== currentUser.tenantId) {
      throw new AuthorizationErrors.NotAllowedAccess();
    }
  }
