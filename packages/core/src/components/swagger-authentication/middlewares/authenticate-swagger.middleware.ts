...
export class AuthenticateSwaggerMiddlewareInterceptor implements Provider<Middleware> {
  ...
  async intercept(context: MiddlewareContext, next: () => ValueOrPromise<InvocationResult>) {
    ...
    if (request && response && this.isOpenAPISpecRequest(request)) {
      ...
      if (!verified) {
        response.status(STATUS_CODE.UNAUTHORISED).setHeader('WWW-Authenticate', 'Basic realm="Node"');
  ...
  private isRequestContext(context?: Context): context is RequestContext {
    return !!((context as RequestContext).request && (context as RequestContext).response);
  }
}
